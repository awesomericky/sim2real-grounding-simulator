...
class GAT_net:
    def __init__(self, real_env, sim_env, args):
        ...
        with tf.variable_scope(self.real_name):
            ...
            self.transformed_next_states = self.build_forward_model('forward')
            ...
            self.fw_loss = 0.5*tf.square(self.real_targets - self.transformed_next_states)
            self.fw_loss = tf.reduce_mean(self.fw_loss)
        ...
        # config.gpu_options.allow_growth = True  ## uncomment it if using GPU is available
    ...
    def build_forward_model(self, name='forward', reuse=False):
        ...
        with tf.variable_scope(name, reuse=reuse):
            ...
            model = tf.layers.batch_normalization(model)
            model = tf.nn.relu(model)
            outputs = tf.layers.dense(model, self.real_state_dim, activation='relu', bias_initializer=param_initializer(), kernel_initializer=param_initializer())
        return outputs
    ...
    def forward_train(self, trajs):
        ...
        fw_loss = self.sess.run(self.fw_loss, feed_dict={self.real_states:states, self.real_actions:actions, self.real_targets:targets})
        return fw_loss
    ...
    def backward_train(self, trajs):
        ...
        bw_loss = self.sess.run(self.bw_loss, feed_dict={self.sim_states:states, self.sim_next_states:next_states, self.sim_targets:targets})
        return bw_loss
    
    def forward_transform(self, state, action):
        state = state.reshape((1, len(state)))
        action = action.reshape((1, len(action)))
        [next_state] = self.sess.run(self.transformed_next_states, feed_dict={self.real_states:state, self.real_actions:action})
        return next_state
